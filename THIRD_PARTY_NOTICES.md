...
VibeVoiceは本人または送信権限を確認できる音声だけを管理者研究経路で扱い、第三者のなりすまし用途へ提供しない。audible disclaimer、watermark、対応detector、hashed inference loggingは別々に実装・検証する必要があり、Microsoft側の説明を根拠にVoice Labのself-hosted runtimeへ実装済みとは表示しない。このため、これらのmitigationが未実装でも一般向けVibeVoiceを公開できるとは判断しない。
