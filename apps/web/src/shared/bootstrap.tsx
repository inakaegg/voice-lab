...
import { applyLocaleToDocument, exposeI18nBridge, lockLocale, setDocumentTitleKey, setReloadOnLocaleChange } from "./i18n";

export function mountPublicPage(
  page: ReactNode,
  legacyScripts: string[] = [],
  options: { localized?: boolean; titleKey?: string; reloadOnLocaleChange?: boolean } = {},
): void {
  ...
  if (options.localized) {
    if (options.titleKey) setDocumentTitleKey(options.titleKey);
    setReloadOnLocaleChange(options.reloadOnLocaleChange === true);
