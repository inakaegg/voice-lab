...
const products = [
  {
    number: "01",
    name: "SpeakLoop",
    titleKey: "portal.speakloopTitle",
    descriptionKey: "portal.speakloopDescription",
    actionKey: "portal.speakloopAction",
    href: "/speakloop",
    icon: Mic2,
  ...
  },
  {
    number: "02",
    name: "Zoovoice",
    titleKey: "portal.zoovoiceTitle",
    descriptionKey: "portal.zoovoiceDescription",
    actionKey: "portal.zoovoiceAction",
    href: "/zoovoice",
    icon: PawPrint,
    ...
    <section className="relative z-10 mx-auto grid w-full max-w-[1180px] min-w-0 flex-1 content-center gap-6 px-5 pb-7 pt-2 sm:px-8 sm:pb-9 min-[900px]:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] min-[900px]:items-center min-[900px]:gap-12 min-[900px]:py-8 lg:gap-16">
      <header className="min-w-0 max-w-[34rem]">
        ...
        <h1 className="text-[clamp(2.65rem,5.6vw,4rem)] font-bold leading-[0.94] tracking-[-0.065em] text-balance">
          <span className="block">{t("portal.headlineLine1")}</span>
          <span className="block">{t("portal.headlineLine2")}</span>
          <span className="block">{t("portal.headlineLine3")}</span>
        </h1>
        <p className="mt-5 max-w-[31rem] text-[0.95rem] leading-7 text-muted-foreground sm:mt-6 sm:text-base sm:leading-8">{t("portal.lead")}</p>
      </header>

      <nav className="min-w-0" aria-label={t("portal.chooseApp")} data-zoovoice-state={zoovoiceEnabled ? "shown" : "hidden"}>
        <Card className="gap-0 overflow-hidden rounded-[1.75rem] border-border/75 bg-card/85 py-0 shadow-[0_28px_80px_rgba(31,38,50,0.11)] backdrop-blur-xl dark:shadow-[0_28px_80px_rgba(0,0,0,0.28)]">
          <CardContent className="p-0">
            {visibleProducts.map(({ number, name, titleKey, descriptionKey, actionKey, href, icon: Icon, tone }) => <a
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <h2 className="m-0 break-keep text-[1.25rem] font-bold leading-tight tracking-[-0.035em] sm:text-[1.45rem]">{t(titleKey)}</h2>
                <span className="mt-2 block text-[0.82rem] leading-[1.65] text-muted-foreground sm:text-[0.9rem]">{t(descriptionKey)}</span>
                <span className="mt-2.5 inline-flex items-center gap-1 text-[0.78rem] font-bold text-[var(--product-accent)] sm:text-[0.82rem]">{t(actionKey)}<ArrowUpRight className="size-3.5" aria-hidden="true" /></span>
              </span>
...
async function startPortal() {
  ...
  // Reactで完結する画面なので、切り替えでページを読み直す必要はない。
  mountPublicPage(<Portal zoovoiceEnabled={zoovoiceEnabled} />, [], { localized: true, titleKey: "portal.pageTitle" });
}
