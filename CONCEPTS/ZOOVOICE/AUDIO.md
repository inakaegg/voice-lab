...
- Stable Audio生成音声24件とCC0移行3件の計27種は旧方針であり、新方針により廃止する。リポジトリからも削除した。
- 現在のセットは実録音を主とする46種である。選別は実録音を生成音より必ず上に置く（`scripts/select_animal_sounds.py`）。以前は生成音のほうが上位にあり、実録音が手元にあるのに使われていなかった。
- 生成音が残っているのはchimpanzee・dolphin・duck・fox・peacock・turkeyの6種で、いずれもまだ実録音を見つけられていない。見つかり次第そこも入れ替え、Stability AI関連の表記も同時に消す。
