...
- 例外として、`CONCEPTS/` 配下は本ファイルの基本コンセプトを詳細化する人間向け仕様ファイルとして新規作成してよい。正本の優先順位は「本ファイルの基本コンセプト → `CONCEPTS/` → `docs/`」とし、矛盾時は上位に合わせて下位を修正する。
...
2. それをASRで文字起こしてして、そのテキストからLLM（現時点ではapi料金が安いgpt Luna）で動物を１つ連想する。連想した理由も取得する。こじつけでもいいので想像力を働かせる。(->GCP cloudrun)
3. その動物（あるいはそれに近い動物）の鳴き声の音声をユーザー音声の始めと終わり部分、さらに無音区間に挿入した音声を出力として返す。(GCP cloudrun->)
