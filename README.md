...
- Usage beyond the free quota can draw on a prepaid credit balance managed by a separate billing service in a private repository. That integration is behind a feature flag, off by default, so this repository runs and is tested on its own; the codebase holds only a thin client and a test fake.
