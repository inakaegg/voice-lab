...
`MO_SPEECH_AUDIO_R2` bindingがある場合、新しい音声履歴blobはR2へ置き、indexとmetadataはKVへ置く。bindingがない場合はKVへfallbackする。各entryが保存先を持つため、旧KV entryと新R2 entryが混在しても取得・削除できる。公開サンプル音声は現時点ではmetadataとbase64音声をKVへ保存する。

KVは簡易デモ向けの保存先であり、大きい音声や長期保存、厳密なquota台帳には使わない。D1 database `mo-speech-demo-db` と `MO_SPEECH_DB` binding、初期migrationは作成済みだが、Workerのquota/audit経路はまだKVを使う。R2 binding、D1 schema、段階移行は [STORAGE.md](STORAGE.md) を参照する。
