...
`GOOGLE_CLIENT_ID` と `GOOGLE_CLIENT_SECRET` は、ポートフォリオ公開時に生成APIをGoogleログイン必須にするためのOAuth clientである。`PUBLIC_SESSION_SECRET` は公開ユーザーのログインcookie署名に使う。未設定の場合は `ADMIN_SESSION_SECRET` にfallbackできるが、公開運用では別値を推奨する。`ADMIN_GOOGLE_EMAILS` は、quota対象外にする管理者Googleアカウントをカンマ区切りで指定する。管理画面側の設定にも管理者メールを追加でき、secret側とKV設定側の和集合を管理者扱いにする。
...
SkitVoiceのURL参照音声取得は、現時点のCloudflare Workerでは未対応である。Worker内では `yt-dlp` と `ffmpeg` を実行できないため、`POST /api/vibevoice/reference-audio-from-url` は `501` を返す。Cloudflare公開版でURL参照音声を使う場合は、RunPod側へURL取得専用operationを追加してWorkerから中継するか、先にローカル/管理環境で参照音声ファイルを用意してアップロードする。
...
求人・ポートフォリオ用途で公開する場合、ページ閲覧自体は誰でも可能にし、OpenAI APIやRunPod課金が発生する生成APIだけをGoogleログイン必須にする。招待コードは職務経歴書から訪問する企業側の初回体験に向かないため使わない。
...
- SkitVoiceの日次/累計回数、台本最大文字数、参照音声最大byte数、URL参照音声の最大取得秒数
