...
- Cloudflare stagingは廃止した。repository側の設定と経路は無い。Cloudflare側のstaging専用のWorker `voice-lab-staging`・D1 `mo-speech-staging-db`・KV `MO_SPEECH_KV_STAGING` も2026-08-09に削除済みで、Cloudflare側にstaging専用resourceは残っていない。
