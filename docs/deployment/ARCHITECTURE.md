...
Voice Labの公開版は、1つのCloudflare WorkerでSpeakLoopとSkitVoiceの非公開案内を配信する。一般ユーザー向け製品はSpeakLoopだけで、SkitVoiceの生成・sample・statusは管理者研究境界へ閉じる。UIはWorker Static Assets、認証・quota・API中継はWorker module、GPU推論はRunPod Serverlessが担当する。この変更はローカル実装で未deployである。
