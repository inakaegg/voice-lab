...
- 一部完了: 公開ポータル、SpeakLoop、SkitVoiceの表示構造とレスポンシブレイアウトをVite + React + TypeScriptへ切替。録音、IndexedDB、比較再生、job pollingは既存controllerをReact mount後に読み込む互換段階で、状態管理のhooks/reducer移行は後続。
- 完了: 公開URLでのOAuth/quota/RunPod実ブラウザE2E確認。
- 素材待ち: 英語・中国語・日本語の公開サンプル生成物登録。
- 進行中: React画面はコンパクト作業台を採用。説明文を圧縮表示し、明色・暗色・システム設定、PC・中間幅・スマホの作業レイアウトをローカルChromeで調整した。公開環境で最終確認後にスクリーンショットを撮影する。
...
5. React画面を実ブラウザで確認してレイアウトを微調整し、スクリーンショットを追加する。
