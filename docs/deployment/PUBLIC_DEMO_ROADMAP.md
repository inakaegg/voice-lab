...
- 完了: 公開UIへ権利確認とプライバシー注意を追加し、SkitVoice生成前の明示確認を必須化。
- 一部実装: D1 database、`MO_SPEECH_DB` binding、初期schemaを作成・deploy済み。R2のproduction/preview bucket、`MO_SPEECH_AUDIO_R2` binding、音声履歴blobのR2保存とKV fallbackも実装済み。quota/auditと公開サンプル音声の移行は未完了。責任境界と段階移行は [STORAGE.md](STORAGE.md) に固定。
- 設計完了・移行未着手: React/TypeScriptへの段階移行境界を [FRONTEND_MIGRATION.md](FRONTEND_MIGRATION.md) に固定。
- 外部確認待ち: 公開URLでのOAuth/quota/RunPod smoke、権利確認済みサンプル登録、実ブラウザのスクリーンショット、CloudflareのD1/R2 binding作成。
...
- SkitVoice生成前に、本人の同意または利用許諾がある参照音声であり、なりすましや誤認を招く用途ではないことを確認する。
...
2. 公開デモのスクリーンショットと権利確認済みサンプル音声を整える。サンプル台本と入力導線は実装済みで、素材登録と撮影は公開環境で行う。
3. ~~D1/R2の保存設計を決め、KVから移す対象を絞る。~~ 設計、D1 resource/schema、R2 bucket/bindingは完了。次はWorkerのquota/auditと公開サンプル音声の移行。
