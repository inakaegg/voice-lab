...
- 一部実装・binding未作成: 音声履歴blobのR2保存とKV fallbackを実装。KV/D1/R2の責任境界、key schema、段階移行を [STORAGE.md](STORAGE.md) に固定。
...
3. ~~D1/R2の保存設計を決め、KVから移す対象を絞る。~~ 設計完了。次はCloudflare binding作成とaudio blobからの段階移行。
