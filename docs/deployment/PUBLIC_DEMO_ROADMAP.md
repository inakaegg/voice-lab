...
- 一部実装: D1 database、`MO_SPEECH_DB` binding、初期schemaを作成・deploy済み。音声履歴blobのR2保存とKV fallbackは実装済みだが、R2はDashboardでのアカウント機能有効化とbucket作成待ち。責任境界と段階移行は [STORAGE.md](STORAGE.md) に固定。
...
3. ~~D1/R2の保存設計を決め、KVから移す対象を絞る。~~ 設計とD1 resource/schema作成は完了。次はR2有効化・bucket作成と、Workerのquota/audit経路移行。
