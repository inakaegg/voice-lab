...
比較の結果、SpeakLoopはコンパクト作業台を正式採用した。説明文は消さず、主要操作までのスクロール量を抑える構成とする。

見た目に関する仕様は [公開UIスタイル方針](../UI_STYLE.md) を正とする。対象は視覚階層・共通部品・必須状態・レスポンシブ境界・テーマ・実画面検証条件である。この文書ではレイアウト仕様を重複管理しない。
...
別の状態として、ログイン、quota表示を持つ。マイク録音、IndexedDB復元、比較再生も別の状態として持つ。これらを1つの巨大なcomponentへまとめず、非同期処理の所有者を明確にする。
...
公開routeの表示層はReactへ切替済みである。残るのは状態管理の移行とする。
...
4. 状態移行の完了後に、対応する旧公開controllerを削除する。
