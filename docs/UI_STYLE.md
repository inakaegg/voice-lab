...
- 通常操作はorb型の録音操作・アニマル度スライダー・動物の種類数・結果の再生／ダウンロードだけとする。
- 調整可能な設定は、アニマル度と動物の種類数だけとする。種類数は1種か2種を選び、既定を1種とする。
- アニマル度スライダーは5段階だけを取る。現在の段階を「3 / 5」の形で示す。
...
- 結果には選ばれた動物、その動物の絵、聞き取った言葉、連想の理由を表示する。
- 動物の絵は鳴き声セットの動物idから引く。対応する絵が無いidは足跡の絵へ落とし、空欄にしない。
- 処理中・token待ち・取消直後・短すぎた録音・Turnstile失敗・エラーの状態を明示する。
...
- 500ms未満で送信しなかった直後
- token待ち（orb無効）
- 生成中（秒数は録音の長さではなく生成開始からの経過時間を数え上げる）
- 生成成功（自動再生）
- ランダムfallback
