...
- SpeakLoopではこのlinkの吹き出しを常時表示にし、hoverなしでデモ動画の存在が分かるようにする。飛び先も公開リポジトリのトップではなくデモ動画の見出しとする。
- デモ動画のlink先は表示言語に合わせて切り替える。日本語表示は日本語READMEの`デモ動画`見出し、英語表示は英語READMEの`Demo videos`見出しへ直接ジャンプさせる。
- ポータルの吹き出しは従来どおりhoverとfocusでだけ表示する。常時表示はSpeakLoopに限る。
