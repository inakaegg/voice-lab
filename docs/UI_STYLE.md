...
- 外部音声処理に関する共通注意文は、SpeakLoopとSkitVoiceで同じフッター部品を使う。短い初期画面ではビューポート下部の左端へ置き、コンテンツが伸びた場合は主要コンテンツ末尾へ続ける。introやサンプル領域へ混在させない。
...
- 復唱の評価後は比較音声を自動再生し、設定用チェックボックスは表示しない。手動の再生と速度調整は維持する。
...
- `自分の声` は録音前に選べる練習設定ツールバーへ置き、既定はオフとする。オンの場合はステップ1の録音をSeed-VC参照音声に使い、変換完了後の音声だけをお手本として自動再生する。処理中は録音操作を無効にし、GPU待機、モデル読込、変換、失敗を既存のjob statusへ表示する。
- 「聞こえた言葉」の差分は単語・文字の脱落だけを `_` で示し、ASRが付けなかった句読点や記号は発音誤りとして表示しない。
- 非同期更新で録音対象、結果、本文、スクロール位置を不用意に動かさない。

### SkitVoice

- 公開サンプルは独立した `出力音声サンプル` セクションとし、表示名は `英語`、`中国語`、`日本語` の順に固定する。PCでは3件を横並びにし、保存元ファイル名は表示しない。
- 参照音声・生成内容の外部処理に関する注意文は、出力サンプルや生成フォーム内へ置かず、SpeakLoopと共通のページフッターへ表示する。
