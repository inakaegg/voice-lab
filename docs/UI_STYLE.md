...
- `自分の声` の横に説明用のoutline iconを置き、hover、keyboard focus、click／tapで「同じセッションで最初に録音した音声からAI生成音声を作る」と短く表示する。外部サービスで処理することは画面共通の注意文へ分離し、長文をtooltipやcheckbox labelへ入れない。
