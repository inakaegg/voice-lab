...
- `自分の声` の近くに、同じセッションの本人録音だけを使うこと、録音と模範TTSを外部処理へ一時送信すること、結果は声質を近づけたAI生成音声であること、Cloudflare版のVoice Lab履歴へ保存しないこと、変換失敗時も通常TTSで続けられることを平易に表示する。長文をcheckbox labelへ詰め込まず、補足文として折り返す。
