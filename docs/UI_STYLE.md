...
- `自分の声` の横に説明用のoutline iconを置き、hover、keyboard focus、click／tapで説明を表示する。説明には、同じセッションの本人録音だけを使うこと、録音と模範TTSを外部処理へ一時送信すること、結果は声質を近づけたAI生成音声であること、Cloudflare版のVoice Lab履歴へ保存しないこと、変換失敗時も通常TTSで続けられることを平易に含める。長文をcheckbox labelや通常画面へ常時表示しない。
