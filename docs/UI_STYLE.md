...
- 通常操作は録音・アニマル度スライダー・Turnstile検証・生成・再生／ダウンロードだけとする。調整可能な設定はアニマル度スライダーだけとする。
- Turnstile検証は録音の後・生成の前に必ず置く手順とする。調整可能な設定ではなく、生成前に完了させる操作と検証として扱う。
- 手動の動物選択、preset、feel lucky、冒頭・間・末尾の調整controlは公開UIへ出さない。
- 結果には選ばれた動物、根拠語、選択方式、ASRテキストを表示する。
- 根拠語がない場合は「該当なし」と示し、空欄にしない。
- 処理中・Turnstile待ち・Turnstile失敗・エラー・ランダムfallbackの状態を明示する。ランダムfallbackでは根拠語がないことを短く示す。
- `1440x900` と `1024x768` は、初期状態とTurnstile widget表示中の録音済み状態の両方で主要領域を1viewport内に収める。対象は録音・アニマル度・Turnstile検証・生成・結果の5領域とする。
