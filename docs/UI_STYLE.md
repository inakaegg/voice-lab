...
- ローカルFastAPI版だけ、折りたたみ式の `過去の結果で表示確認` を設定の下へ表示する。成功済みの復唱履歴を現在の結果UIへ復元し、表示時に外部APIや音声処理を呼び出さない。保存結果の表示中は復唱録音を無効にする。
- 保存履歴からお手本音声を一意に特定できない場合は推測で対応付けない。比較再生を無効にし、保存した復唱音声だけを再生可能にする。
