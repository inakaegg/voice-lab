...
- 中国語比較ASRではFunASRの認識本文を補正せず、元のword timestampを`raw_start`／`raw_end`として保持する。targetとのlexical alignmentと、元音声から独立に検出した0.12秒以上の無音が同じ境界を支持する場合だけtimestampを精密化する。raw timestampを無音候補の優先根拠に使うのは、phraseの未割当と未割当の非フィラー音声が同時に残る場合、または信頼できる文末記号が低一致のlexical境界を近傍で訂正する場合に限る。句順、他targetとの非競合、文字根拠が不足する場合はraw timestampまたは`available=false`を維持し、無関係発話へ高信頼rangeを捏造しない。RunPod handlerは一時音声が存在する間に、受け取ったお手本音声と復唱音声の両方へこの処理を適用する。timestamp精密化は、クライアントがお手本音声としてどの音声を選択するかとは独立して扱う。
