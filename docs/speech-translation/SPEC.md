...
- 中国語比較ASRではFunASRの認識本文を補正せず、元のword timestampを`raw_start`／`raw_end`として保持する。targetとのlexical alignmentと、元音声から独立に検出した0.12秒以上の無音が同じ境界を支持する場合だけtimestampを精密化する。句順、非フィラーchunk、他targetとの非競合、文字根拠が不足する場合はraw timestampまたは`available=false`を維持し、無関係発話へrangeを捏造しない。RunPod handlerは一時音声が存在する間にお手本と復唱の両方へこの処理を適用し、声質変換後の音声を比較ASR入力にしない。
