...
- 発音練習用画面では模範音声の再生速度をYouTube相当の `0.25x` から `2.0x` まで、`0.25x` 刻みのスライダーで選べる。速度変更は既に生成済みの音声をブラウザ再生速度で変えるだけで、TTS再生成はしない。選択した速度はブラウザ側に保存し、音声ロード後と再生直前に保存値を再適用する。
- 比較再生は、文単位の `句点` 分割と、カンマや読点も含める `句読点` 分割をUIで選べる。既定は `句読点` とし、中国語の短い節を比較しやすくする。復唱結果が出た後は、模範音声の再生ボタンを比較再生ボタンへ切り替え、同じボタンで停止できるようにする。復唱ASRにword/segmentタイムスタンプがあり、復唱テキストが学習対象言語として妥当な場合だけ、復唱音声を文ごとに切って比較し、ボタン文言は `文ごと比較再生` とする。タイムスタンプが無い、または中国語復唱なのに漢字がほとんど無いなどASR結果が崩れている場合は、文字数比で無理に区切らず、模範音声全体と復唱音声全体の比較再生へ戻し、ボタン文言は `全体比較再生` とする。
...
- `自分の録音` や `もう一度` の専用小ボタンは置かない。同じ目標文をもう一度練習する場合は、画面上部の単一マイクをそのまま復唱入力として使う。次の内容へ進む場合だけ、小さい `次の文` 操作で現在の目標文をクリアし、同じマイクを母国語入力へ戻す。
