...
- `自分の声` のトグル横には、hover、keyboard focus、click／tapで確認できる説明iconを置き、「同じセッションで最初に録音した音声からAI生成音声を作る」と短く案内する。外部サービスで音声処理することは画面共通の注意文で案内し、詳細な保存・保持条件はprivacy文書を正とする。第三者の同意をcheckboxで証明できるとは扱わない。
