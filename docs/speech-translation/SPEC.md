...
`/fun` は管理者認証済みの場合だけ表示・利用でき、公開ポータルには導線を置かない。Cloudflare版では、有効な管理者セッションを`/fun`の生成API認証としても扱い、Google OAuthを重ねて要求せずquotaを消費しない。ただし入力サイズ上限は管理者にも適用する。ローカルFastAPIは開発者が起動する信頼済み環境として、管理ログインなしで管理画面と`/fun`を提供する。
