...
- SpeakLoopの録音UIは、`1 言いたいことを話す` と `2 練習` の2ステップに分ける。目標文がまだ無い状態ではステップ1の母国語入力マイクを赤くし、録音後に母国語ASR結果を同じステップ内へ `言ったこと` として表示する。目標文ができた後はステップ1のマイクをグレー、ステップ2の練習マイクを赤にする。ステップ1のカードへホバーまたはステップ1マイクへフォーカス/タップした場合はステップ1を赤、ステップ2をグレーに切り替える。赤く見えるマイクは常に1つだけにする。録音送信は従来どおり `practice recordings` APIへ集約し、サーバー側の自動分類を維持する。
- 発音練習用画面の録音UIは、トップページと同じ録音ボタン、録音中の波形/レベルメーター、処理中アニメーションを再利用する。独自の録音ボタンで視覚フィードバックを弱くしない。
- 発音練習用画面の固定文言は、ひらがなのみの日本語にはしない。公開ユーザー画面では説明文を増やさず、`言いたいことを話す`、`練習`、`言ったこと`、`聞こえた言葉` のような短い日本語を主に使う。
- 発音練習用画面では、ASRモデル選択UIを出さず、`whisper-1` を固定で使う。理由は、SpeakLoopの売りであるフレーズごとの比較再生にword/segmentタイムスタンプが必要であり、通常の公開UIで全体比較へ落ちる選択肢を見せると機能差が分かりにくいためである。
- 発音練習用画面では、OpenAI Transcriptions APIへ `whisper-1`、`verbose_json`、`timestamp_granularities` を指定し、word/segmentタイムスタンプを取得する。API互換用に `gpt-4o-transcribe` と `gpt-4o-mini-transcribe` も受けられるが、公開ユーザー画面には出さない。これらのモデルを内部検証で使う場合は、タイムスタンプを要求せず、JSON応答から文字起こし本文だけを使い、タイムスタンプ欄は未対応として扱う。
- 発音練習用画面では模範音声の再生速度を `0.5x` から `2.0x` まで、`0.1x` 刻みのスライダーで選べる。速度変更は既に生成済みの音声をブラウザ再生速度で変えるだけで、TTS再生成はしない。選択した速度はブラウザ側に保存し、模範音声、復唱音声、比較再生の各再生直前と再生開始直後に保存値を再適用する。
- 比較再生の分割単位は、ユーザー画面では選択UIを出さず、目標文側の句読点フレーズを内部既定にする。復唱結果が出た後は、模範音声の再生ボタンを比較再生ボタンへ切り替え、同じボタンで停止できるようにする。復唱ASRにword/segmentタイムスタンプがあり、目標文フレーズとの単調アラインメントが成立した場合だけ、復唱音声をフレーズごとに切って比較し、ボタン文言は `フレーズごと比較再生` とする。タイムスタンプが無い、短すぎる断片しか取れない、または中国語復唱なのに漢字がほとんど無いなどASR結果が崩れている場合は、文字数比で無理に区切らず、模範音声全体と復唱音声全体の比較再生へ戻し、ボタン文言は `全体比較再生` とする。`練習終了後すぐ再生` は既定ONとし、復唱結果が返ったら現在の比較再生モードで自動再生する。ユーザーがOFFにした場合はブラウザ側に保存し、次回以降は手動再生にする。
- 発音練習用画面は、高速に練習を回すため、PC/スマホともにできるだけ1画面内へ収める。メイン画面には現在の学習対象言語、母国語入力マイク、模範文、練習マイク、再生速度、比較再生、判定を近い位置に置く。言語選択は画面上部の国旗付きドロップダウンにし、歯車型の設定ボタンは置かない。説明文や大きいカードで縦に伸ばさない。
- 発音練習用画面は、前回選んだ学習対象言語、中国語のピンイン表示ON/OFF、模範音声の再生速度をブラウザ側のlocalStorageなどへ保存し、次回ロード時の既定値にする。初回の学習対象言語は `en-US` とし、選択肢は `🇺🇸 English`、`🇨🇳 中文` の順に表示する。保存済み学習言語が現在の公開UI対象外（`ja-JP`など）の場合も `en-US` へ正規化する。中国語のピンイン表示ON時は、可能な限り漢字ごとの `ruby` 表示として漢字の上に置く。
- 発音練習用画面では、母国語入力のASR結果も表示する。ユーザーが「自分が何と言ったと認識されたか」を確認できるようにする。
- 公開サンプル音声はCloudflare版だけでなくローカルFastAPI版でも同じ管理APIから登録・取得・削除できる。ローカル版は `MO_PUBLIC_SAMPLE_AUDIO_PATH`（既定 `tmp/public-sample-audios.json`）へ保存し、管理画面と公開画面で同じJSON契約を使う。
- SkitVoiceの公開サンプル領域は `出力音声サンプル` を見出しとし、`英語`、`中国語`、`日本語` の固定名・固定順で表示する。PCでは3件を横並びにし、保存元ファイル名は公開画面・管理画面とも表示しない。外部API送信と履歴保存の注意文はサンプル領域へ併置しない。
- SkitVoiceのサンプル、参照音声、生成結果、行ごとの中間音声、管理画面と履歴の音声操作はVoice Lab共通プレイヤーへ統一し、ブラウザ既定のaudio controlsを直接表示しない。
- SkitVoiceの公開サンプルと管理画面プレビューは同一のカスタム音声コントロールを使い、再生／一時停止、シーク、経過時間／総時間を提供する。ネイティブaudio controlsは表示せず、ボタンとrange inputへ音声名を含むアクセシブルなラベルを設定する。
- 管理画面の保存・削除など非同期操作は、押下直後に対象ボタンを処理中表示かつ無効状態にし、完了時はボタンと `aria-live` 状態欄の両方で成功または失敗を通知する。状態欄が長いフォームの末尾にしか見えない構成にしない。
...
- 復唱結果は、お手本文の直下に同程度の大きさで表示し、目標文との差分で不一致と判断した部分を赤字で目立たせる。差分箇所はクリック可能にする。認識結果に存在するが目標文と違う語句は赤字にし、その直上に小さく正解語句を表示する。復唱で抜け落ちた目標文側の語句は `抜け:` などの説明文を付けず、赤い下線付きの正解語句として挿入する。お手本と聞こえた言葉は可能な範囲で同じ並びになるよう、差分表示は認識結果側の挿入位置を保持する。比較再生では、復唱ASRのword timestampと目標文側フレーズの対応が取れた場合だけ「1フレーズ目のお手本→1フレーズ目の復唱、2フレーズ目のお手本→2フレーズ目の復唱」の順で再生する。条件を満たさない場合は、文字量から無理に復唱音声を分割せず、模範音声全体と復唱音声全体の順に再生する。
- `自分の録音`、`もう一度`、`次の文` のような専用小ボタンはユーザー画面に置かない。同じ目標文をもう一度練習する場合は、`2 練習` のマイクを使う。次の内容へ進む場合は、`1 言いたいことを話す` のマイクを使い、サーバー側の自動判定で新しい母国語入力として扱えるようにする。
- 復唱用マイクは `2 練習` ステップ内に置き、お手本テキストの右横へ表示する。ステップ1とステップ2のどちらを押しても同じ `practice recordings` APIを使うが、表示上は次に押すべきマイクだけ赤くする。
- SkitVoice画面は、`zhskit` のRunPod Web UI相当の生成機能を公開向けに簡素化した画面として、台本テキストまたは台本ファイル、最大4つの音声/動画参照ファイル、ブラウザのマイク録音、CFG scale、inference steps、seed、temperature、top_p、top_k、line-by-line結合を指定できるようにする。テキストファイルを選んだ場合は内容を台本テキストエリアへ読み込み、送信前に編集できるようにする。台本が空の初回表示では、最小例として `1 あっ、こんにちは〜`、`2 こんにちは。ご無沙汰してます。`、`1 元気ですか。どうしてました？` の3行を入れておく。台本は `Speaker 1: ...` 形式を正とし、話者タグがない入力は各非空行を `Speaker 1:` として補う。入力の手間を減らすため、`1: ...`、`1 ...`、`A: ...`、`A ...` の短縮タグも受け、生成前に `Speaker N:` へ正規化する。短縮タグではコロンは任意、タグと本文の間には半角スペースを置く。短縮タグはUIの参照音声数に合わせて `1-4` または `A-D` を扱い、英字タグは `A=1`、`B=2` のように話者番号へ変換する。参照音声は可能なら `ffmpeg` で24kHz mono WAVに正規化してからVibeVoiceへ渡す。SkitVoice画面で選択または録音した参照音声はブラウザ内のIndexedDBへ保存し、次回以降は同じSpeaker枠の既定音声として表示・送信できるようにする。file input自体にはブラウザ制約で前回ファイル名を直接再セットすることはできないため、保存済みファイル名を枠内に表示し、保存済みBlobをaudioコントロールで再生確認できるようにする。Cloudflare公開版ではWorker内で `yt-dlp` / `ffmpeg` を実行せず、YouTubeなどのURL参照はbot確認やログイン要求で不安定なため公開ユーザー画面に出さない。Cloudflare WorkerのSkitVoice生成APIも `voice_url_1` から `voice_url_4` を受け取った場合はRunPodへ送らず拒否する。URLからの切り出しはローカルFastAPI版または管理者の事前素材作成用に残し、ローカルFastAPIが切り出し済み音声として通常の参照音声へ変換してから生成へ渡す。RunPod handlerはURLを受け取らず、`audio_base64` の参照音声だけを扱う。クラウドアップロードは初期実装では扱わない。
- SkitVoice画面ではVibeVoiceへ渡す台本の出力言語を選択できる。候補は `en-US`、`zh-CN`、`ja-JP` で、UI表示は `🇺🇸 English`、`🇨🇳 中文`、`🇯🇵 日本語（低品質）` とする。`日本語台本を出力言語へ翻訳` がONの場合は、生成前に台本本文だけを選択言語へ翻訳する。翻訳では話者タグ、非空行数、行順を保持し、翻訳後台本をVibeVoice生成、行ごと生成、指定台詞ASR再配置の入力に使う。翻訳はOpenAI Responses APIを使い、モデルは `OPENAI_VIBEVOICE_SCRIPT_TRANSLATION_MODEL`、未指定時は `OPENAI_TRANSLATION_MODEL`、さらに未指定時は `gpt-5.5` とする。診断には元台本、翻訳後台本、出力言語、翻訳モデルを含める。
- SkitVoice画面のレイアウトは、PCでは台本編集を左、参照音声、生成設定、実行ボタンを右へまとめるコンパクトな2カラムを基本とし、画面内で台本編集から生成までを見渡せるようにする。ログイン状態はSpeakLoopと同じくヘッダー右上へコンパクトに置き、主操作を押し下げない。モバイルでは1カラムへ落とすが、余白と見出しを詰め、不要な説明文で操作面が押し下げられないようにする。生成パラメータの数値はスライダーで調整でき、現在値を隣に表示する。実行環境の詳細表示はデバッグ用途なので上部には常時表示せず、画面下部の折りたたみ詳細として確認できるようにする。
- SkitVoice画面からの生成はジョブAPIで実行し、生成中は現在ステージ、経過時間、キャンセルボタンを表示する。ローカル実行のジョブは固定timeoutで停止せず、キャンセルではCLI subprocessを終了する。互換用の同期リクエストはtimeoutで500へ落とさず、明示的なtimeoutエラーを返す。完了後は生成にかかった時間を表示する。実際に生成へ使った参照音声のSpeaker枠、ファイル名、サイズを診断情報へ含め、保存済み音声や録音音声の意図しない混入を確認できるようにする。ローカルFastAPI版でURL参照を使う場合は、元URL、開始秒、切り出し秒数も診断情報へ含める。
...
- `audio`: SpeakLoop画面で録音した音声ファイル。UI上はステップ1/2のマイクを分けるが、APIは同じ音声入力として受け取る。
...
- `asr_model`: 公開ユーザー画面では `whisper-1` 固定。API互換用に `gpt-4o-transcribe`、`gpt-4o-mini-transcribe` も受けられるが、通常UIには出さない。
...
- `grade_label`: 画面表示用の短い判定文。`0.995` 以上は `できました`、`0.95` 以上は `いいかんじ`、`0.90` 以上は `まあまあ`、それ未満は `もう一回`。
