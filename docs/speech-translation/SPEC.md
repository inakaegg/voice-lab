...
- 比較再生は目標文の句読点をフレーズ境界とし、ASRのword timestamp列へ順序を保って全体最適化で対応付ける。単一フレーズは明確な文頭・文末フィラーだけを除いた発話全体を使う。複数フレーズでは、高信頼anchor間の非フィラーtokenを重複なく前後いずれかへ割り当て、抜け、挿入、言い間違い、どもり、言い直しを比較区間から理由なく落とさない。内容が大きく違っても、pause、前後anchor、発話順から目標フレーズの位置を特定できる区間は低信頼で再生可能にする。構造上の根拠がない無関係な発話へ区間を捏造しない。
- 各比較区間は、実timestampがあり再生できるかを `available`、内容一致を `content_matched` と `content_similarity`、位置の信頼度を `alignment_confidence`、境界根拠を `boundary_source` で別々に表す。`complete=true` は全フレーズが再生可能で、説明不能な内部の非フィラー未割当tokenがない場合に限る。
