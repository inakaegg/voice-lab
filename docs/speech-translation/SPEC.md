...
- 画面はユーザー用、発音練習用、管理者用に分ける。
  ...
  - `/vibevoice` はユーザー向けのVibeVoiceかんたん生成画面とし、台本テキストと最大4つの話者参照音声から、指定台詞向けの既定設定で音声を生成してブラウザ上で確認できるようにする。
  - `/vibevoice/admin` はVibeVoiceの検証・管理画面とし、backend、モデル、sampling、ASR再配置、低スコア再生成などの詳細設定を表示する。VibeVoice機能は初期実装ではローカル/RunPod検証用であり、Cloudflare本番Workerには載せない。
