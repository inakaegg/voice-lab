...
`/fun` は管理者認証済みの場合だけ表示・利用でき、公開ポータルには導線を置かない。ローカルFastAPIは開発者が起動する信頼済み環境として、管理ログインなしで管理画面と`/fun`を提供する。
...
| 公開音声blob | ローカルファイル | R2、bindingなし時のみfallback | job結果のみ |
...
- 公開デモでは設定に応じて短い音声履歴を保存する。機密情報を入力しないよう画面で案内する。
