...
- SpeakLoopではSeed-VC、ジョーク、大阪弁、バリエーションを使わない。速度を優先し、OpenAI ASR、OpenAI翻訳、OpenAI TTSだけで模範音声を作る。母国語入力は `auto` ASR、復唱入力は選択された学習対象言語をASRへ渡す。初期判定は発音音響スコアではなく、ASRで聞き取れた文と目標文の正規化文字列類似度で行う。
