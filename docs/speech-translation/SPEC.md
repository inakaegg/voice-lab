...
- 復唱録音のASRは言語別に固定する。中国語 `zh-CN` は同じRunPod Serverless endpointのFunASR `paraformer-zh`、英語 `en-US` はOpenAI `whisper-1` を使う。母語で「言いたいことを話す」録音は言語自動判定が必要なため、従来どおりOpenAI ASRを使う。
...
| SpeakLoop中国語復唱ASR | provider経由で依頼 | 音声bytesを同期jobとして中継 | FunASR `paraformer-zh` |
...
RunPod handlerはURL、cookie、ブラウザ認証情報を受け取らず、音声bytesだけを受け取る。SpeakLoop中国語復唱では `operation_mode=practice_asr` と `audio_base64` を受け取り、認識本文とtimestampだけを返す。URL取得失敗時はRunPod処理が始まっていないため、RunPodを原因として表示しない。
