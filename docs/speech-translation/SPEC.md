...
- RunPod Serverless backendのwarm状態は `/api/runtime` の `runpod_serverless` backend情報に含める。ユーザー用画面では、RunPod backendが選ばれている場合だけwarm状態を短い表示で出す。完全なscale-to-zeroでは初回にcold startが入るため、必要なら録音前または管理者操作から `warmup` operationを実行してモデルロード済み状態へ寄せる。RunPod `/health` の `IDLE` や `READY` はworker存在の参考情報にとどめ、Seed-VC ready表示はwarmupまたはSeed-VC job成功によって短時間保存されたready状態だけを根拠にする。
