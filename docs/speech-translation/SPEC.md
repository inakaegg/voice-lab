...
- 中国語の発話比較では表示文字列を保持したまま、targetとASR本文の両方へ同じspoken-form正規化を適用する。一般整数、年、小数、割合、温度、時刻、`v`付きversion、compact identifierを文脈別に扱い、数値変更、単位や小数点の脱落、一般数量のdigit-by-digit読みを同一視しない。一般整数は4桁groupと`万`以上の単位を保った通常の中国語数詞へ変換し、桁数が大きくてもJavaScript `Number`の丸めで値を変えない。時刻のhourは`03`を`三`として扱う一方、minuteの`05`は`零五`を保持する。`v1.2.3`のような複数componentのversionは、先頭componentを通常数詞、後続componentを先頭ゼロ保持のdigit読みとして、すべての区切りを`点`へ変換する。負の温度、割合、小数、整数は`-`またはUnicode minus `−`を`负`として保持し、同じ絶対値の正数と同一視しない。`零下`表現は自動的に同一視しない。
...
- 比較ボタンの表示と実際の再生方式は同じplanを使う。全フレーズ対応は `フレーズごと比較再生`、一部対応は `一部フレーズ比較再生`、対応なしは `全体比較再生`、`no_speech` は `お手本を再生` とする。比較可能な復唱結果では比較ボタンとは別に `お手本だけ再生` を表示し、いつでもお手本単独へ戻れるようにする。「聞こえた言葉」の差分は対応するpaired phrase rangeがある場合だけ操作可能にし、クリック時はそのフレーズの先頭からお手本と復唱を比較再生する。先頭からの全体比較へ黙ってfallbackしない。区間再生はAPIの `audio_end` ちょうどで停止し、固定の早止めやpaddingを加えない。
...
RunPod handlerはURL、cookie、ブラウザ認証情報を受け取らず、音声bytesだけを受け取る。SpeakLoop中国語復唱では `operation_mode=practice_asr`、`audio_base64`、`model_audio_base64`、`target_text` を受け取り、両音声の認識本文とtimestampを返す。`model_audio_base64` は通常TTSのお手本音声とし、`自分の声` がオンでもSeed-VC後の再生音声へ差し替えない。両音声比較のhandler outputは `practice_asr_contract_version=2` と `model_transcription` を必須とし、WorkerとFastAPIは旧imageの欠落を一般的なASR失敗と混同せず再デプロイ案内にする。`自分の声` では既存の `operation_mode=voice_conversion` を使い、WorkerまたはFastAPIがSpeakLoop専用job APIとして状態を中継する。RunPodのprogress updateは途中stage表示に使い、最終job outputを採点と比較再生の正、Seed-VC job outputを再生音声の正とする。URL取得失敗時はRunPod処理が始まっていないため、RunPodを原因として表示しない。
