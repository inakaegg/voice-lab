...
- ユーザー用画面の出力には、通常の日本語テキストと、表示補助用のひらがなテキストを並べて表示する。ひらがな化は日本語出力時だけ行い、初期実装ではOpenAI Responses APIを使う。
- ユーザー用画面ではブラウザ標準のaudio controlsを主UIにせず、出力音声の再生/停止を専用トグルボタンで表示する。
