...
- 中国語の発話比較では表示文字列を保持したまま、targetとASR本文の両方へ同じspoken-form正規化を適用する。一般整数、年、小数、割合、温度、時刻、`v`付きversion、compact identifierを文脈別に扱い、数値変更、単位や小数点の脱落、一般数量のdigit-by-digit読みを同一視しない。一般整数は4桁groupと`万`以上の単位を保った通常の中国語数詞へ変換し、桁数が大きくてもJavaScript `Number`の丸めで値を変えない。時刻のhourは`03`を`三`として扱う一方、minuteの`05`は`零五`を保持する。
