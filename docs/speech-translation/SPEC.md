...
- 録音または音声処理の実行中は、履歴からの表示確認を開始できない。
- 履歴からの表示確認では、保存時点の採点結果であることを画面に明示する。古い履歴に声調判定用ピンインがない場合は、ローカルの決定的な変換処理で補う。
- 履歴からの表示確認では、お手本生成時に保存した `tts_text` と復唱結果の目標文が完全一致するお手本音声を対応付ける。同じ目標文のお手本が複数ある場合は、その復唱より前に作られた最も新しいものを選ぶ。あいまい一致や時刻の近さだけによる推測では選ばない。
- 対応するお手本音声を保存履歴から見つけられた場合は、通常の復唱結果と同じ比較再生を復元する。見つけられない場合は比較再生を無効にし、復唱音声と結果表示だけを復元する。
- 保存結果の表示中は、どちらの場合も復唱録音を無効にする。
- 履歴からの表示確認は、比較区間を `保存値` と `現行ロジックで再計算` のどちらで再生するかを選べる。既定は `保存値` とする。
...
- 音声長は保存済み音声ファイルから求め、取得できない場合はASR単語の終了時刻で代用する。実装本体と同じ順序にする。
