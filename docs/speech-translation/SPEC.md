...
- 複数phraseの所有判断はfalse positiveを重く見る。単一phrase、前後の文字anchorで一意に挟まれたspan、順序どおりの完全一致segment等、数値化済みの根拠で位置を特定できる場合だけ割り当てる。片側anchor、pause、空いているslot、録音端だけを理由に無関係な発話へ区間を作らない。どもり、言い直し、自己訂正は、所有できたphrase内部から理由なく落とさない。
