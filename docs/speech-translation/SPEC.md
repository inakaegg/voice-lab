...
- ユーザー用画面の右上には表示変換トグルを置き、画面全体の文言と出力テキストを `🇯🇵 ひらがな`、`🇯🇵 ルビ`、`🇮🇩 Indonesia` の順に3段階で切り替える。ひらがな化とインドネシア語表示は日本語出力時にOpenAI Responses APIで作る。出力がインドネシア語の場合、インドネシア語表示は出力テキストをそのまま使う。
