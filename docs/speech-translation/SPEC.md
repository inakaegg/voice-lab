...
- ユーザー用画面ではブラウザ標準のaudio controlsを主UIにせず、出力音声の再生/停止を専用トグルボタンで表示する。出力後にトグルが変更された場合、再生ボタンは作り直し操作として扱い、最後の録音音声を現在のトグル設定で再処理して最新出力を置き換える。
- ユーザー用画面の音声翻訳は、翻訳方式を `OpenAI API` 固定、入力言語を `auto`、出力言語を既定 `ja-JP` とする。出力言語は管理者用画面のユーザー画面設定で変更できる。
- ユーザー用画面の `にてるこえ` トグルがONの場合は `voice_mode=convert` とし、Seed-VCを使う。Seed-VCの参照音声は入力音声自身、`seed_vc_reference_auto_select=true` 固定とする。
