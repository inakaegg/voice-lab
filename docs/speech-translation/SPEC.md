...
- 中国語 `zh-CN` では、目標文と復唱ASR本文の文字ごとの声調つきピンインをサーバー側で生成し、置換として検出された文字対をこのピンインで再判定する。声調まで完全一致する場合は誤りとして強調せず（同音の別字としてASRが選んだだけとみなす）、音節が一致し声調だけ違う場合は「声調のみの違い」として通常の誤りと異なる見た目で示す。文脈依存の多音字（例: 副詞の「地」）はこの判定の対象外になり得る。
