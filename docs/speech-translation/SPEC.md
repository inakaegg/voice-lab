...
- 比較再生は目標文の句読点をフレーズ境界とし、ASRのword timestamp列へ順序を保って全体最適化で対応付ける。単一フレーズは明確な文頭・文末フィラーだけを除いた発話全体を使う。複数フレーズでは、抜け、言い間違い、どもり、言い直しを比較区間から落とさず、後続フレーズのtimestampを奪わない範囲で冒頭のリスタートと末尾の反復も含める。timestampがない区間を文字数比で捏造しない。
