...
- 整列後はVADスナップをかける。発話島ごとに重なる認識単位を集め、その島の先頭認識単位の開始時刻と末尾認識単位の終了時刻だけを対象にする。島端との差が±0.35秒以内の場合だけ、その時刻を島端へ差し替える。
- VADスナップはフレーズ境界を入力に取らない。フレーズの再生区間が変わるのは、そのフレーズの開始または終了が差し替え済みの認識単位に当たる場合だけである。
...
- 再生余白は選択範囲の前後へ足し、直前の認識単位の終了時刻と直後の認識単位の開始時刻、音声全体の長さでクランプする。
