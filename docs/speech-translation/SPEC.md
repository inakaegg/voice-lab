...
- 発音練習用画面は、前回選んだ学習対象言語、中国語のピンイン表示ON/OFF、模範音声の再生速度をブラウザ側のlocalStorageなどへ保存し、次回ロード時の既定値にする。保存済み学習言語が現在の公開UI対象外（`ja-JP`など）の場合は `zh-CN` へ正規化する。中国語のピンイン表示ON時は、可能な限り漢字ごとの `ruby` 表示として漢字の上に置く。
