...
- `自分の声` を選ぶ前から、本人録音と模範TTSを外部処理事業者へ一時送信すること、結果が本人の声質に近づけたAI生成音声であること、Cloudflare版のVoice Lab履歴には保存しないことを、トグル横の説明iconからhover、keyboard focus、click／tapで確認可能にする。第三者の同意をcheckboxで証明できるとは扱わない。
...
- 比較ボタンの表示と実際の再生方式は同じplanを使う。全フレーズ対応は `フレーズごと比較再生`、一部対応は `一部フレーズ比較再生`、対応なしは `全体比較再生`、`no_speech` は `お手本を再生` とする。比較可能な復唱結果では比較ボタンとは別に `お手本だけ再生` を表示し、いつでもお手本単独へ戻れるようにする。区間再生はAPIの `audio_end` ちょうどで停止し、固定の早止めやpaddingを加えない。
