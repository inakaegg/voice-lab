...
- アプリは、フレーズを連結すると元の目標文になること、位置番号、元の時刻、余白適用後の時刻、点数の範囲を検査する。比較再生に使う認識文字列は、LLMが返した位置番号に対応するASR文字列から作る。LLMが返した `matched_text` は処理判断に使わない。フレーズ分割、点数、位置番号、時刻を作り直さない。
