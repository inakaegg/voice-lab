...
- LLMは位置番号、元の時刻、前後共通余白を加えた再生時刻まで含む完成JSONを返す。誤答や言い直しは、一致した部分だけへ狭めず、対応する発話全体を再生区間へ含める。
- アプリは、フレーズを連結すると元の目標文になること、位置番号、認識文字列、元の時刻、余白適用後の時刻、点数の範囲を検査する。フレーズ分割、点数、時刻を作り直さない。
- 比較モデルは `gpt-5.6-terra`、`gpt-5.6-luna`、`gpt-5.4-mini`、`gpt-5.4-nano` から選ぶ。前後共通余白は0.00〜0.50秒を0.05秒刻みで選び、お手本と復唱の両方へ同じ値を使う。
- LLM呼び出しまたは返却値の検査に失敗した場合、従来処理へ切り替えず「比較結果を作成できませんでした。もう一度お試しください。」と表示する。
- ローカルFastAPI版はこの処理に対応済みである。Cloudflare公開版への反映と、従来処理専用コードの削除は [ROADMAP.md](ROADMAP.md) の未完了項目とする。
...
- ローカルFastAPI版は、お手本作成と復唱評価も非同期jobとして実行する。お手本の文字起こし、翻訳、通常TTS、両音声のASR、LLMによる比較結果作成を別の処理段階として返し、各段階に実際のサービス名とモデル名を含める。公開UIは主要文言の直下にこれらを小さく薄い技術詳細として表示する。
