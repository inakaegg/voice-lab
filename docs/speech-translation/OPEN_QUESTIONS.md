...
- 現行実装は、翻訳で確定した目標文 `target_text` と、学習者の復唱録音をASRした本文 `recognized_text` を比較してscore、grade、diffを計算する。お手本音声のASR本文 `model_recognized_text` は、お手本側のphrase割当と再生区間の取得に使うが、採点計算には使わない。
...
- 採点対象は現時点では変更しない。変更を検討する場合は、目標文、お手本ASR本文、復唱ASR本文を揃えた複数の実録音で現行方式、ASR本文同士の比較、併用方式を同じデータ上で比較し、改善数と悪化数を確認してから判断する。
