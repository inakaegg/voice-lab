...
- 中国語の時刻はforced alignment（fa-zh）で求め直し、発話島の縁に近い認識単位の時刻を縁へスナップする。
...
| VADスナップ | 発話島ごとに先頭と末尾の認識単位の時刻を、縁との差が±0.35秒以内なら縁へ差し替える仕上げ処理。 |
...
この評価サンプルでは、fa-zhの出力に時刻がわずかに早めへ出る傾向を観測した。スナップは、発話島ごとに重なる認識単位を集める。対象はその島の先頭認識単位の開始時刻と末尾認識単位の終了時刻だけで、縁との差が±0.35秒以内ならその時刻を縁へ差し替える。

スナップはフレーズ境界を入力に取らない。フレーズの再生区間が変わるのは、そのフレーズの開始または終了が差し替え済みの認識単位に当たる場合だけである。差し替えるのは `words` の時刻だけで、認識単位の順序とLLMへ渡す位置番号は変えない。
...
| --- | ---: | ---: | ---: |
| ASRの時刻をそのまま使う（baseline） | 59% | 68% | 1.77秒 |
| fa-zh＋VADスナップ（採用） | 89% | 89% | 1.24秒 |

0.25秒以内の割合が68%から89%へ変わったこと、最大距離が1.77秒から1.24秒へ変わったことは、いずれもこの指標上の値である。計測に使った音声はリポジトリに含まれないため、この数値をそのまま再現することはできない。
