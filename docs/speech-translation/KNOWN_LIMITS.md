...
- Seed-VCの類似度と自然さは参照音声の長さ、雑音、話し方に依存する。本人の同意または利用許諾がある音声だけを使う。
...
- ブラウザがタブ共有を許可しても、コンテンツの利用許諾を意味しない。自分の音声、許諾済み音声、またはライセンス上利用可能な音声だけを参照音声に使う。

## 保存とプライバシー

- ローカルFastAPI版はローカル音声履歴を保存できる。公開Cloudflare版は設定に応じて短い入力・出力音声と公開サンプルをWorkers KVへ保存する。
- Workers KVは厳密なatomic incrementを持たないため、現在のquotaは公開デモの過剰利用防止であり、課金台帳ではない。
- KVは大きい音声blobの長期保存、厳密な監査、リレーショナルな検索には適さない。音声履歴blobは任意のR2 bindingへ保存できるが、公開サンプル音声のR2移行と、quota・監査・job metadataのD1またはDurable Objects移行は未完了。
