...
unit除外reasonは少なくとも`non_numeric`、`non_finite`、`negative_start`、`end_before_start`、`beyond_audio_duration`を区別する。primitive不正を1件以上除外したsourceは`invalid_timestamp_unit`をflagへ残す。source全体のflagは少なくとも`non_monotonic_timestamp_source`、`duplicate_timestamp_unit`、`overlapping_timestamp_units`、`word_segment_boundary_conflict`を区別する。浮動小数の比較許容差は[GAP-P2-03](#gap-p2-03-時刻比較の許容誤差)で別に固定し、許容差を補正値としてcanonical rangeへ書き込まない。
...
各軸の採用候補だけを結合し、integration challenge setで確認する。今回利用できた第1弾200件、第2弾100件は、いずれも仕様統合または実装比較の過程で内容・出力を確認済みである。このため、pilot 20/20、segment policy 12/12、round2 challenge 76/76を「最後まで未使用の評価setでの精度」とは呼ばない。

contract version 1の移行可否は、固定期待値への適合、既存回帰、Python/Worker parity、API/UI契約、性能budgetで判断した。次にownershipまたはcontent policyを変更する場合は、変更案を実行する前に新しい実録音を確保し、入力、期待値、除外条件、SHA-256を固定した未使用canonical evaluation setを必須gateとする。既存300件を名前だけ変えてheld-outへ戻さない。
...
production baselineは常時正規化後の文字anchor、単一phrase、前後anchorで一意なspan、固定済みsegment-only規則だけを決定的に使用する。pause等を将来追加する場合は正例・反例・境界例、両runtime parity、独立評価を伴うcontract version更新を必須とする。
...
| --- | ---: | ---: | ---: |
| Python 4 phrase / 16 token | 11.389 / 11.583ms | 10.803 / 10.995ms | traced peak 165,404 bytes |
| Python 16 phrase / 64 token | 337.103 / 371.110ms | 331.282 / 354.129ms | traced peak 3,929,181 bytes |
| Python 1 phrase / 256 token | 27.213 / 28.124ms | 27.362 / 27.544ms | traced peak 179,021 bytes |
| Worker 4 phrase / 16 token | 4.335 / 5.857ms | 4.034 / 5.381ms | GC後heap差 205,856 bytes |
| Worker 16 phrase / 64 token | 150.915 / 152.983ms | 138.603 / 142.456ms | GC後heap差 27,240 bytes |
| Worker 1 phrase / 256 token | 14.397 / 14.603ms | 13.160 / 14.284ms | GC後heap差 30,592 bytes |
...
- canonical evaluationはPython/Workerともpilot 20/20、segment policy 12/12、round2 challenge 76/76で一致した。
- [SPEC.md](SPEC.md)、schema、Python、Worker、fixture、test、UI、[LEARNING_ROADMAP.md](LEARNING_ROADMAP.md)を同じ変更単位で同期した。
- `python3 -m pytest`は909件、`npm test`は255件、Playwrightは1440px、1024px、390pxの99件が成功した。
