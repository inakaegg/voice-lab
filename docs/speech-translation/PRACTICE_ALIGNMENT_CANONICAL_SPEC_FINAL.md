...
2. 除外後の同一source内に非単調な順序、同一unitの重複、正時間range同士の交差が残る場合、そのsource全体をownershipと再生時刻へ使用しない。並べ替え、重複除去、境界短縮で救済しない。
3. wordsが安全に使用できる場合はwordsをownershipの第一sourceとする。segmentsがwordsと矛盾する場合、segmentsをboundary evidenceにも使用せずflagを残す。wordsとsegmentsを同じ発話として二重計上しない。
...
unit除外reasonは少なくとも`non_numeric`、`non_finite`、`negative_start`、`end_before_start`、`beyond_audio_duration`を区別する。source全体のflagは少なくとも`non_monotonic_timestamp_source`、`duplicate_timestamp_unit`、`overlapping_timestamp_units`、`word_segment_boundary_conflict`を区別する。浮動小数の比較許容差は[GAP-P2-03](#gap-p2-03-時刻比較の許容誤差)で別に固定し、許容差を補正値としてcanonical rangeへ書き込まない。
...
最終結果はPython 18/20、Cloudflare Worker 18/20である。2件の差は期待値未確定ではなく、独立判定済みの無関係発話を片側anchorと空きslotだけで所有させるfalse positiveである。
...
現行のcount-only fallbackはPython 6/12、Cloudflare Worker 6/12で、同じ6件に失敗した。失敗はすべて、数の一致をownership根拠にしたこと、または正時間を検査しなかったことによる。よってcount-only fallbackを不採用とし、[10.8](#108-segment-onlyの固定baseline)を`ownership.conservative`へ統合する。
...
| `tests/fixtures/practice_alignment_canonical/round2_challenge_expectations.json` | `acdc558aceb15039409b1f8f74c744c4e8dd566a4aac1c6429700a761f22ec85` |

overlayは旧手作業rangeのうちcanonical coreと矛盾しないものだけをfield変換し、次を本文書から上書きした。実装出力から期待値を逆算していない。

- `partial_overlap_negative`の独立判定
- `available=false`とraw timestamp payloadが矛盾する2件のprovider error
- segment数が一致しなくても、一意な完全一致segmentだけを所有できる2件
- word/segment index、zero-duration owner、未割当ownership unit数

分類は正常結果74件、provider contract error 2件、`ambiguous_excluded` 4件である。PythonとCloudflare Workerはともに固定76件中70件に一致し、同じ6件に失敗した。

| 差 | 件数 | 現行挙動 |
| --- | ---: | --- |
| confirmed unrelated speechの誤割当 | 1 | 時刻だけが共通する第2発話をphraseへ所有させる |
| contradictory timestamp payload | 2 | provider errorにせず通常の再生不能結果を返す |
...
pilotと合わせた独立判定12件では、両runtimeとも`relevant_attempt` 5/5を所有できた一方、`unrelated_speech` 0/3で3件すべてを誤割当した。`ambiguous` 4件は除外した。これは現行方式が低一致発話を一律に捨てる問題ではなく、片側anchor後の発話を内容にかかわらず所有させる問題である。したがって単なるsimilarity閾値変更へ進まず、confirmed unrelated speechを除外する構造条件を先に実装・再評価する。

[18.4](#184-停止条件)の「confirmed unrelated speechを新たに割り当てる」に該当するため、現行方式のまま未使用canonical evaluation setへ進まない。
...
残作業はAPI schema、Python/Worker、UI表示fixtureへの実装反映であり、errorを成功resultへ混ぜる方式へ戻さない。

### GAP-P0-03 不正timestampの扱い

状態: `closed`

[5](#5-timestampの妥当性)の保守契約を採用する。単独unitのprimitive異常はunit除外、source内の非単調・重複・交差はsource全体を使用不能、wordsとsegmentsの矛盾は安全なwordsを優先してsegmentsを根拠から外す。補正、並べ替え、clamp、時刻捏造は行わない。

正式transcriptionが非空なら安全なtimestampだけで制限付き評価を続け、すべて使用不能かつtranscriptionも空ならprovider契約違反とする。残作業は異常種別fixture、Python/Worker parity、音声実長検査の実装反映である。
