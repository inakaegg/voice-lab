...
Seed-VCへ渡す `target` は、アップロード音声そのものではなく、サーバー側で短いmono WAVへ正規化した参照音声にする。ブラウザ録音はWebM/Opusになりやすく、無音や余分な長さを含むとSeed-VCの処理時間が大きく伸びるためである。既定では先頭12秒までを参照音声として使い、`SEED_VC_REFERENCE_MAX_SECONDS` と `SEED_VC_REFERENCE_SAMPLE_RATE` で調整できる。
