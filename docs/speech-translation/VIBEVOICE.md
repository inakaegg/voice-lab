...
実装は段階的に進める。現在のWeb UIでは、1話者の「改行・空白を1行化して生成」だけを生成設定として選べる。この設定は、表示用台本を変更せず、生成前の正規化で同一話者の複数行を句読点つきの1行へまとめ、行ごと生成とは同時に使わない。複数話者でONにした場合は、まだ話者別生成と再配置が未実装であるためエラーにする。次にASR分割と無音挿入、最後に複数話者の話者別生成と再配置へ進む。複数話者の再配置では、ASR結果の誤認識、台詞の脱落、生成順のずれ、長尺生成の途中終了を検出してユーザーへ確認できるUIが必要になる。
