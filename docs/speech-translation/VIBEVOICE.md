...
- `出力言語`: VibeVoiceへ渡す台本の言語を選ぶ。初回の既定値は `英語(en-US)` とし、候補は `英語(en-US)`、`中国語(zh-CN)`、`日本語(ja-JP・低品質)` とする。ブラウザに保存済みの選択がある場合は、その値を復元して既定値で上書きしない。VibeVoiceは英語・中国語で比較的安定し、日本語は研究用途の範囲では不安定なため、UI上でも低品質候補として明示する。
