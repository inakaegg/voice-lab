...
指定台詞向けモードの1行化テキストは、話者ごとに短すぎても長すぎても崩れやすい。現時点の経験則では、VibeVoiceへ実際に渡す採用対象テキストは1チャンクあたりおおむね140〜180文字程度を実用範囲として扱い、末尾ガード込みのVibeVoice投入テキストは280文字程度までに抑える。140文字未満の場合は、末尾欠落対策のガード文を必要長まで増やす。話者ごとの合計が180文字を超える場合は、話者内で複数チャンクへ自動分割し、各チャンクを `VibeVoice -> Seed-VC -> ASR` に通してから元台本順へ再配置する。1行だけで180文字を超える場合は、行単位では安全に分割できないため入力エラーにする。Web UIも送信前に同じ上限で単一行の過長を検出し、自動分割される話者がある場合は送信時メッセージに表示する。これは固定の品質保証ではなく暫定境界値であり、診断情報にはチャンクごとの採用対象文字数、ガード文字数、最終投入文字数、範囲外フラグを残す。
