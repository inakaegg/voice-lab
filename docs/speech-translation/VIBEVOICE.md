...
実装は段階的に進める。最初は1話者の「1行化して生成」だけをUIで試せるようにし、次にASR分割と無音挿入、最後に複数話者の話者別生成と再配置へ進む。複数話者の再配置では、ASR結果の誤認識、台詞の脱落、生成順のずれ、長尺生成の途中終了を検出してユーザーへ確認できるUIが必要になる。
