...
def create_user_display_text(text: str, target_language: str) -> dict[str, str]:
    ...
    return asdict(
        UserDisplayText(
            ...
            indonesian_text=_indonesian_with_openai(kanji_text),
        )
    )
...
def _indonesian_with_openai(text: str) -> str:
    if not os.getenv("OPENAI_API_KEY"):
        raise RuntimeError("OPENAI_API_KEY is required for Indonesian display text.")
    try:
        from openai import OpenAI
    except ImportError as exc:
        raise RuntimeError("openai package is required for Indonesian display text.") from exc

    response = OpenAI().responses.create(
        model=os.getenv("OPENAI_TEXT_DISPLAY_MODEL", os.getenv("OPENAI_TEXT_TRANSFORM_MODEL", "gpt-5.5")),
        instructions=(
            "Translate the Japanese sentence into natural Indonesian for display to language learners. "
            "Return only the Indonesian text, with no notes."
        ),
        input=text,
    )
    return _text_from_response(response)
