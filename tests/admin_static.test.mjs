...
// 表示文言は辞書へ移した。文言そのものは辞書ファイルで確かめる。
const i18nMessages = await readFile(new URL("../apps/web/src/shared/i18n-messages.ts", import.meta.url), "utf8");
...
test("SpeakLoop controller keeps pronunciation training separate from conversion demo", () => {
  ...
  assert.match(practiceSource, /case "transcribing_prompt":[\s\S]*speakloop\.practice\.stepTranscribing/);
  assert.match(practiceSource, /case "translating_prompt":[\s\S]*speakloop\.practice\.stepTranslating/);
  assert.match(practiceSource, /case "synthesizing_prompt":[\s\S]*speakloop\.practice\.stepSynthesizing/);
  assert.match(practiceSource, /case "evaluating_comparison":[\s\S]*speakloop\.practice\.stepBuildingComparison/);
  ...
  assert.match(practiceSource, /speakloop\.practice\.comparisonFailed/);
  assert.match(i18nMessages, /"speakloop\.practice\.comparisonFailed": "比較結果を作成できませんでした。もう一度お試しください。"/);
  ...
  assert.match(practiceSource, /speakloop\.practice\.pinyinUnavailable/);
  assert.match(i18nMessages, /"speakloop\.practice\.pinyinUnavailable": "ピンインを生成できませんでした"/);
  ...
  assert.match(practiceSource, /practiceText\("speakloop\.practice\.recordingCancelled"\)/);
  assert.match(i18nMessages, /"speakloop\.practice\.recordingCancelled": "録音をキャンセルしました。"/);
  assert.match(practiceSource, /function pausePlaybackForRecording[\s\S]*modelAudio\.pause\(\)[\s\S]*repeatAudio\.pause\(\)/);
  assert.match(practiceSource, /practiceText\("speakloop\.practice\.playThisPhrase"\)/);
  assert.match(i18nMessages, /"speakloop\.practice\.playThisPhrase": "このフレーズを比較再生"/);
  assert.match(practiceSource, /practiceText\("speakloop\.practice\.playMissingPhrase"\)/);
  assert.match(i18nMessages, /"speakloop\.practice\.playMissingPhrase": "抜けたフレーズを比較再生"/);
  ...
  assert.match(practicePlaybackSource, /playbackText\("speakloop\.playback\.compareWhole"\)/);
  assert.match(i18nMessages, /"speakloop\.playback\.compareWhole": "全体比較再生"/);
  assert.match(practicePlaybackSource, /playbackText\("speakloop\.playback\.comparePartial"\)/);
  assert.match(i18nMessages, /"speakloop\.playback\.comparePartial": "一部フレーズ比較再生"/);
  assert.match(practicePlaybackSource, /playbackText\("speakloop\.playback\.comparePhrases"\)/);
  assert.match(i18nMessages, /"speakloop\.playback\.comparePhrases": "フレーズごと比較再生"/);
