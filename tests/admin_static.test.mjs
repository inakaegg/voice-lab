...
test("SpeakLoop controller keeps pronunciation training separate from conversion demo", () => {
  ...
  assert.match(practiceSource, /t\("speakloop\.practice\.recordingCancelled"\)/);
  ...
  assert.match(practiceSource, /t\("speakloop\.practice\.playThisPhrase"\)/);
  assert.match(i18nMessages, /"speakloop\.practice\.playThisPhrase": "このフレーズを比較再生"/);
  assert.match(practiceSource, /t\("speakloop\.practice\.playMissingPhrase"\)/);
