...
test("practice page keeps pronunciation training separate from conversion demo", () => {
  ...
  assert.match(practiceHtml, /value="whisper-1" selected/);
  assert.match(practiceHtml, /whisper-1（フレーズ比較）/);
  assert.match(practiceHtml, /通常は whisper-1/);
  assert.match(practiceHtml, /gpt-4o\/mini は全体比較再生/);
  assert.match(practiceHtml, /whisper-1 はタイムスタンプ取得時だけフレーズごと比較再生/);
  ...
  assert.match(practiceSource, /openPracticeSettings/);
  assert.match(practiceSource, /closePracticeSettings/);
