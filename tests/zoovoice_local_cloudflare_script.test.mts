...
test("Zoovoice local launcher dry-run uses Go and Wrangler without starting FastAPI or child commands", () => {
  ...
  try {
    ...
    assert.match(result.stdout, /wrangler d1 migrations apply MO_SPEECH_DB .*--local.*--persist-to/);
    assert.match(result.stdout, /ZOOVOICE_PORT=8090 go run \./);
    assert.match(result.stdout, /wrangler dev .*--local.*--ip 127\.0\.0\.1.*--port 8787/);
...
test("Zoovoice Cloud Run launcher dry-run uses redacted service account impersonation", () => {
  const serviceAccount = "must-not-appear@example-project.iam.gserviceaccount.com";
  ...
  assert.match(result.stdout, /--audiences=https:\/\/zoovoice-example-uc\.a\.run\.app/);
  assert.match(result.stdout, /--project=example-project/);
  assert.match(result.stdout, /ZOOVOICE_ORIGIN_MODE=cloud-run-smoke/);
  assert.doesNotMatch(result.stdout + result.stderr, new RegExp(serviceAccount.replaceAll(".", "\\.")));
  assert.doesNotMatch(result.stdout + result.stderr, new RegExp(secretValue));
  assert.doesNotMatch(result.stdout + result.stderr, /ZOOVOICE_GCP_SERVICE_ACCOUNT_JSON|PRIVATE KEY/);
});
...
test("Zoovoice Cloud Run launcher fails before child commands when required inputs are missing", () => {
  for (const omitted of [
    ...
    const values: Record<string, string> = {
      ZOOVOICE_CLOUD_RUN_URL: "https://zoovoice-example-uc.a.run.app",
      ZOOVOICE_GCP_PROJECT: "example-project",
      ZOOVOICE_SMOKE_SERVICE_ACCOUNT: "smoke@example-project.iam.gserviceaccount.com",
    };
    ...
    assert.doesNotMatch(result.stdout + result.stderr, /wrangler dev|go run|npm run build:web/);
  }
});
...
test("package scripts expose both Zoovoice Wrangler modes", () => {
  const local = spawnSync("npm", ["run", "dev:zoovoice"], {
    cwd: repositoryRoot,
    env: { ...process.env, ZOOVOICE_DRY_RUN: "1" },
    encoding: "utf8",
  });
